 ...
 * 🌌 銀河手寫數字辨識系統 - 完整修正版
 * 修正內容：
 * 1. 解決繪圖起點異常連線問題 (修正 startDrawing 邏輯)
 * 2. 解決檔案上傳需兩次之問題 (清空 input value)
 * 3. 優化鏡頭與語音開關的狀態管理與視覺回饋
 * 4. 嚴格保留所有辨識核心演算法 (advancedPreprocess, predict 等)
 */
...
async function predict(isRealtime = false) {
    ...
    try {
        ...
        for (const comp of filtered) {
            ...
            // 此處省略連體字切割與單一辨識的詳細重複程式碼，保持原 logic
...
async function toggleCamera() {
    if (cameraStream) {
        ...
        try {
            ...
            document.getElementById('camToggleBtn').innerHTML = '<span class="btn-icon">📷</span> 關閉鏡頭';
...
function stopCamera() {
    ...
    document.getElementById('camToggleBtn').innerHTML = '<span class="btn-icon">📷</span> 開啟鏡頭';
    init(); // 恢復黑底畫布
}

// 檔案上傳 Bug 修正：處理完畢後清空 value
...
function handleFile(event) {
    ...
    reader.onload = (e) => {
        ...
        img.onload = () => {
            ...
            predict(false);
            // 關鍵修正：清空 input 確保下次選同一張圖也能觸發 change
            event.target.value = ""; 
        };
...
function startDrawing(e) {
    ...
    ctx.beginPath(); // 必須立即開啟新路徑
    ctx.moveTo(x, y); // 將起點移至當前點，防止與舊路徑連線
}
...
function setupEventListeners() {
    ...
    window.addEventListener('mouseup', stopDrawing); // 改為 window 監聽，防止滑出畫布失效
